...
Stand: 03.09.2026
Masterstand: V21 (Projektstabilisierung)
Projektversion: 0.19.0
Backend-Version: 0.19.0
Ziel: bestehenden V20-Online-Stand auf Basis des bestätigten V21-Buildstands stabil weiterentwickeln – NICHT neu aufsetzen.
...
### Projektstabilisierung V21
- alle bekannten `possibly undefined`-Fehler unter `strict` und `noUncheckedIndexedAccess` ohne Funktionsabbau behoben
- Expo-SDK-57-Abhängigkeiten auf die erwarteten kompatiblen Versionen abgeglichen
- fehlende direkte Peer-Abhängigkeit `expo-asset` ergänzt
- TypeScript 6.0.3 und React Native 0.86.3 validiert
- vollständiger TypeScript-Check erfolgreich
- Backend-Syntaxcheck erfolgreich
- Expo Doctor: 21/21 Prüfungen erfolgreich
- Expo-Web-Export erfolgreich
- Android- und iOS-JavaScript-/Hermes-Bundles erfolgreich erzeugt
- ein echter signierter Development-/Release-Build auf Geräten bleibt separat offen

### GitHub und öffentliche Web-Version V21
...
- V21 wird über denselben GitHub-Actions-Workflow aus `main` veröffentlicht
...
CODEX_MASTER_VERKAUFSASSISTENT_V21.md
README.md
app.json
package.json
pnpm-lock.yaml
...
18. Native Expo-Push-Benachrichtigungen sind in der Browser-Version nicht verfügbar.

---

## 12. Nächste Prioritäten für Codex

### PRIORITÄT A – Projekt stabil bauen – V21 abgeschlossen

Am 03.09.2026 bestätigt:

1. bestehende TypeScript-Fehler ohne Funktionsverlust behoben.
2. `pnpm run typecheck` erfolgreich.
3. Expo Doctor mit 21/21 erfolgreichen Prüfungen.
4. `pnpm run build:web` erfolgreich.
5. Android-Bundle erfolgreich.
6. iOS-Bundle erfolgreich.
7. `node --check backend/server.mjs` erfolgreich.

Noch kein Ersatz für diese gesonderten Prüfungen: signierter Development-/Release-Build und End-to-End-Test auf echten Android-/iOS-Geräten.
...
### Bestätigte Validierung des V21-Stabilisierungsstands
...
[x] vollständiger TypeScript-Check grün
[x] Expo Doctor 21/21 grün
[x] Backend-Syntaxcheck grün
[x] Android-Bundle erfolgreich
[x] iOS-Bundle erfolgreich
...
> **„Arbeite PRIORITÄT B der CODEX_MASTER_VERKAUFSASSISTENT_V21.md kontrolliert in der eBay Sandbox ab. Nutze nur echte, manuell bereitgestellte Zugangsdaten, prüfe OAuth, Publish, Inbox, Webhook, Push und `withdrawOffer` end-to-end und dokumentiere jeden bestätigten Schritt.“**
...
2. `CODEX_MASTER_VERKAUFSASSISTENT_V21.md`
3. `CODEX_MASTER.md`
4. `CODEX_MASTER_VERKAUFSASSISTENT_V20.md`
5. aktuelle `README.md`
6. ältere Versionen / alte Chatverläufe

Ältere ZIPs und V19-/V20-Masterdateien niemals als aktuellen Stand behandeln, wenn V21 vorhanden ist.
